...
var payModule = (function(BrowserEnabled, GestPay, configuration, util) {
  ...
  function start3DSecureVerification(result) {
    ...
    var redirectUrl = location.origin + baseUrl + '/pay-secure';
	  var gestpaySecureUrl = `${gestpay3dUrl}?a=${configuration.shopLogin}&b=${VBVRisp}&c=${redirectUrl}`;
	  console.log(`redirecting the user to ${gestpaySecureUrl} ...`);
	  document.location.replace(gestpaySecureUrl);
  }
  ...
  function redirectToResponsePage(result) {
	  var redirectUrl = 'response?a=' + configuration.shopLogin + '&b=' + result.EncryptedString;
